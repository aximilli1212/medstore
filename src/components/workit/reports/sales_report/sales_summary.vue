<template>
    <div>
        <v-btn @click="$router.push('/all_invoice_list')" color="secondary" dark small class="mb-2"><v-icon left>mdi-plus</v-icon> View Invoice List</v-btn>
        ...
        <v-card class="thead">
            <v-card-title>
                ...
                <p class="headline ttext mt-3">All Sales Sessions [ <i class="green--text text--darken-3">{{total}}</i> ]</p>
                <v-spacer></v-spacer>
                <v-text-field
                        v-model="search"
                        append-icon="search"
    ...
    export default {
        ...
        methods:{
            ...
            loadSesData(){
                ...
                axios.post(this.apiSet.allSession,sesUp)
                    .then(response=>{
