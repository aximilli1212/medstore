<template>
    <div>
        <v-dialog v-model="transfer_dialog" persistent max-width="1200px">
            ...
            >
                <v-card-title>
                    <p class="headline ttext mt-1">[ <i class="green--text text--darken-3">{{transfer_total}}</i> ] Received Products | </p><br />
                    ...
                    >
                        mdi-delete
                    </v-icon> Reject All</v-btn>


                    <v-btn large class="secondary elevation-8" @click="acceptItems"><v-icon>mdi-format-align-middle</v-icon> Accept Items</v-btn>
                ...
                >
                    <template slot="items" slot-scope="props">
                        ...
                        <td slot="activator">{{ props.item.purr_quantity+'('+props.item.ws_unit+')/'+props.item.purr_rt_quantity+'('+props.item.rt_unit+')'  }}</td>
        ...
        <v-card class="thead">
            <v-card-title>
                ...
                <p class="headline ttext mt-3">Received Stock[ <i class="green--text text--darken-3">{{total}}</i> ] from <i class="green--text text--darken-3">{{outlet}}</i></p>
                <v-spacer></v-spacer>
                <v-text-field
                        v-model="search"
                        append-icon="search"
                ...
                <template slot="items" slot-scope="props" >
                    ...
                    <td class="justify-center layout px-0">
                        ...
                        <v-btn
                                ...
                                @click="recView(props.item)"
                        ><v-icon
                        >
                            mdi-arrow-down
                        </v-icon> Receive</v-btn>
    ...
    export default {
        ...
        data(){
            return{
                ...
                purchase_head: [
                    ...
                    { text: 'No. Items', align: 'left', value: 'num_items',class:'subheading' },
                    { text: 'Destination', align: 'left', value: 'source' ,class:'subheading'},
                    { text: 'Source', value: 'destination' ,class:'subheading'},
                    { text: 'Sender', value: 'sender' ,class:'subheading'},
                    { text: 'Status', value: 'status',class:'subheading' },
        ...
        watch:{
            ...
            outlet(val){
                ...
                if(!val || val === "all Outlets"){

                    this.purchases = original;
                    this.total = this.purchases.length;
                    this.outlet = "all Outlets";
                }
                else{
                    this.purchases = _.filter(original, function(o) { return o.destination === val; });
                    this.total = this.purchases.length;
                }
        ...
        methods:{
            ...
            viewDet(purr_item){
                ...
                axios.get(this.apiSet.reorderBuffer+'/'+purr_item.reorder_invoice)
                    .then(response=>{
                        ...
                        self.fetchup = false;
                        self.$router.push('/pos_reorder_invoice');
            ...
            loadReceived(){
                ...
                // Nii change
                // const r={
                //     source:this.userData.outlet_name,
                ...
                axios.post(this.apiSet.postReorderTransfered, r)
                    .then(response=>{
                        const obj = response.data.data;
            ...
            acceptItems(){
                ...
                for (let key = 0, len = p.length; key < len; key++){
                    // p[key].outlet_id = this.userData.outlet_id;
                    // p[key].outlet_id = 2;

                    axios.post(this.apiSet.upReorder, p[key])
