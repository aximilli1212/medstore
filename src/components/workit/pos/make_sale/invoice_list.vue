<template>
    <div>
        <v-btn @click="$router.push('/my_sales')" color="primary" dark small class="mb-2"><v-icon left>mdi-cash</v-icon> View My Sales</v-btn>


        <v-card class="thead">
            <v-card-title>
                <v-btn icon color="primary" @click="$router.go(-1)" small><v-icon>mdi-arrow-left-thick</v-icon></v-btn>
                <p class="headline ttext mt-3">My Invoice List [ <i class="green--text text--darken-3">{{total}}</i> ]</p>
                <v-spacer></v-spacer>
                <v-text-field
                        v-model="search"
                        append-icon="search"
    ...
    export default {
        ...
        methods:{
            ...
            loadSesData(){
                ...
                axios.post(this.apiSet.post_csales,sesUp)
                    .then(response=>{
