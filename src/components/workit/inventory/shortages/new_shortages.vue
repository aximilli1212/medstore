<template>
    <div>
        <v-dialog v-model="pull_dialog" persistent max-width="500px">
            <v-card>
                <v-toolbar >
                    <v-toolbar-title class="primary--text">Pull Minimum</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-btn icon @click.native="pull_dialog = false">
                    ...
                    <v-container grid-list-md>
                        <v-layout wrap>
                            <v-flex xs12 sm12 md12>
                                <v-text-field box v-model="min_quantity" label="Enter Minimum Quantity to pull"></v-text-field>
                            </v-flex>
                        </v-layout>
                    </v-container>
                </v-card-text>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn class="secondary elevation-4" @click.native="fetchMin">Pull less than {{min_quantity}}  <v-icon right>mdi-arrow-down</v-icon></v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="dialog" persistent max-width="900px">
            <v-card>
                <v-toolbar >
                    <v-toolbar-title class="primary--text">Reorder Product <v-btn flat color="secondary">{{editedItem.product_name}} #{{editedItem.id}}</v-btn></v-toolbar-title>
        ...
        <v-card color="primary pa-2 round_end">
            <v-layout row align-center>
                ...
                <!--</v-flex>-->
                    ...
                    <v-tooltip
                            top
                    >
                        <v-btn   slot="activator"  dark  color="orange darken-3" class="mb-2 ml-4" :loader="loader" :disable="loader" @click="autoFetch"><v-icon left>mdi-arrow-collapse</v-icon>Auto Pull Low Stock</v-btn>
                          <span>click to retrieve all minimum quantity products.</span>
                    </v-tooltip>
                    <v-tooltip top>
                        <v-btn slot="activator" dark  color="grey darken-3" class="mb-2" :loader="loader" :disable="loader" @click="pullMin"><v-icon left>mdi-arrow-down</v-icon>Pull Minimum </v-btn>
            ...
            <v-layout>
                ...
                <v-flex sm2 md2>
                    <v-btn dark color="secondary" @click="$router.push('/inv_shortage_view')"><v-icon left>mdi-view-carousel</v-icon>Preview</v-btn>
                </v-flex>
                <v-flex sm2 md2>
                    <v-btn dark  color="green darken-3" :loader="loader" :disable="loader" @click="$router.push('/inv_shortage_view')"><v-icon left>mdi-redo-variant</v-icon>Generate Reorder</v-btn>
        ...
        <v-card class="thead">
            ...
            >
                <template slot="items" slot-scope="props">
                    ...
                    <td>{{ props.item.available_qty }}</td>
    ...
    export default {
            ...
            getNewPurchase(){
                ...
                for (let key = 0, len = obj.length; key < len; key++) {
                    dict.push({
                        ...
                        available_qty:obj[key].ws_quantity+' ('+obj[key].ws_unit+') / '+obj[key].rt_quantity+' ('+obj[key].rt_unit+')',
        ...
        methods:{
            ...
            pullMin(){
                ...
                // setNewPopPurchase  --- Mutate his ass
                this.pull_dialog = true;
            ...
            fetchMin(){
                ...
                let self = this;
                this.fetchup = true;
                axios.post(this.apiSet.minReorder, min)
                    .then(response=>{
                        const obj = response.data.data;
                        self.$store.commit('setNewPopPurchase',obj);
                        self.fetchup = false;
                        self.pull_dialog = false;
            ...
            autoFetch(){
                ...
                let self = this;
                this.fetchup = true;
                axios.post(this.apiSet.post_low_stock, min)
                    .then(response=>{
                        const obj = response.data.data;
                        self.$store.commit('setNewPopPurchase',obj);
                        self.fetchup = false;
            ...
            submitPurchase(){
                ...
                let rev =  this.userData.company_name.charAt(0)+'-' + Math.random().toString(36).substr(2, 16);
                        ...
                        for (let key = 0, len = p.length; key < len; key++) {
                            ...
                            axios.post(this.apiSet.reorderBuffer, p[key])
                                .then(function (response) {
                                    self.loader = false;
                                    self.$validator.reset();
                                    self.$store.commit('setSnack',{t:(key+1)+" Product(s) Ordered. ",c:"green darken-3"});
