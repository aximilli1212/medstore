<template>
    <div>
        ...
        <v-btn color="secondary" @click="$router.go(-1)" small><v-icon left>mdi-printer</v-icon>Print</v-btn>
        <v-card color="pa-4 mt-2 round_end">



            <header>
                <h1>Purchase Invoice <i class="secondary--text">#{{purchaseDetail.sup_invoice}}</i></h1>

                <address style="font-size:15px" >
                    <p class="headline"> Supplier: {{purchaseDetail.supplier}}</p>
                    <p> Mode: {{purchaseDetail.payment_mode}}</p>
                    <p>  Discount: {{purchaseDetail.sup_discount}}</p>
                    <p>  Vat: {{purchaseDetail.vat}}</p>
                    <p> Total Cost Price: {{purrSubtotal}}</p>
                    <p> Total Selling Price: {{purrSub}}</p>
                    <!--<tr>-->
                        <!--<td>#Invoice No.</td>-->
                        <!--<td style="font-weight:bold">{{purchaseDetail.sup_invoice}}</td>-->
                    <!--</tr>-->
                </address>
                <span class="mid" >
        ...
        computed:{
            ...
            purrSub(){
              let p = this.$store.getters.getNewPurchase;
                ...
                let total_amount = p.reduce((t, n) => t + parseFloat(n.purr_subtotal), 0);
                return total_amount;
            }
        },
    }
